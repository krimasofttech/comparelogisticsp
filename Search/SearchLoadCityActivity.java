...
public class SearchLoadCityActivity extends AppCompatActivity implements CommonInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_search_load_city);
        initComponent();
    ...
    public void handleIntentCall() {
        if (getIntent() != null) {
            ...
            if (intent.hasExtra(SearchLoadActivity.fromcityText)) {
                citykey = "fromcity";
                key = citykey;
                b = true;
                spinner.setSelection(0);
                requestedCode = SearchLoadActivity.fromcitycode;
            } else if (intent.hasExtra(SearchLoadActivity.tocityText)) {
                citykey = "tocity";
                key = citykey;
                b = true;
                spinner.setSelection(0);
                requestedCode = SearchLoadActivity.tocitycode;
            } else if (intent.hasExtra(SearchLoadActivity.truckText)) {
                key = "truck";
                b = true;
                spinner.setSelection(1);
                requestedCode = SearchLoadActivity.trucktextcode;
            }
        }
    }
    ...
    public void setFilterAdapter() {
        final ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this,
                R.array.filtersload, R.layout.mytextview);
// Specify the layout to use when the list of choices appears
    ...
    public void changeSearch(String data) {
        if (data.contains("City")) {
            ...
            searchView.setHint("Search Load");
    ...
    //it will change adapter to loadAdapter
    public void changeToTruckAdapter() {
        searchFlag = "truck";
        truckAdapter = new LoadAdapter();
    ...
    public ArrayList<String> getTrucks(ArrayList<String> list) {
        String[] trucks = getResources().getStringArray(R.array.loadcategory);
        for (String s : trucks) {
            list.add(s);
        }
        return list;
    ...
    public void startSearch(Handler mThreadHandler, String value) {
        if (new ConnectionUtility(SearchLoadCityActivity.this).isConnectingToInternet()) {
    ...
    public void handleCityCall(String city) {
        Intent intent = new Intent(this, SearchLoadActivity.class);
